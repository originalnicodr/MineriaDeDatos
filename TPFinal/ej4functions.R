    ...
    #for (c in c(10^(-5),10^(-4),10^(-3),10^(-2),10^(-1),1,10,10^2,10^3,10^4,10^5)){
        for(g in 10*(1:40)){
            ...
            for (fold in 1:k){
                ...
                modelo <-svm(formula = Casualty.Severity~., data = training_set, kernel = "radial", cost = c, gauss=g, scale = FALSE)   
                prediction<-predict(modelo, newdata = testing_set)
                #print(prediction)
                #print(training_set[,"N_tipo"])
                error<-1-sum(prediction==testing_set[,classname])/dim(testing_set)[1]        
    ...
    #for (c in 1:10){
        for(d in 1:10){
            ...
            for (fold in 1:k){
                training_set <- data[folded_data$.folds != fold,]
                testing_set <- data[folded_data$.folds == fold,]        
                modelo <-svm(formula = Casualty.Severity ~ ., data = training_set, kernel = "radial", cost = c, scale = FALSE, degree=d)       
                #print(modelo)
                prediction<-predict(modelo, newdata = testing_set)
                error<-1-sum(prediction==testing_set[,classname])/dim(testing_set)[1]        
                performances<-c(performances,error)
...
crossvalidate_boosting <- function(data, k = 10, classname="N_tipo"){
    ...
    for (i in 1:20){
        ...
        for (fold in 1:k){
            ...
            #colnames(training_set)[ncol(training_set)] <- "class"
            #colnames(testing_set)[ncol(testing_set)] <- "class"


            adaboost <- boosting(Casualty.Severity~., data=training_set, mfinal=200,
                             coef="Freund",
...
crossvalidate_randomforest <- function(data, k = 10, classname='N_tipo'){
    ...
    for (i in 1:(ncol(data)-1)){
        ...
        for (fold in 1:k){
            ...
            test_confussion_temp<-randomForest(x=data[,-class_index], y=data[,class_index],  ntree=200, mtry= sqrt(ncol(data)-1) )$confusion
