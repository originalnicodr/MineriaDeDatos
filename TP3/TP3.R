...
#aplico k-means
cckmeans<-kmeans(scale(crabs[,4:8]),cent=4)
#plot(crabs[,4:8],pch=as.numeric(crabs[,2]),col=cc$cluster)
plot(crabs[,4],pch=as.numeric(crabs[,2]),col=cckmeans$cluster)
abline(v=0)
...
lampone_prc<-prcomp(lampone[,-c(dim(lampone)[2],143, 1)])

plot(lampone_prc,col=as.numeric(lampone[,1]),pch=as.numeric(lampone[,143]))
